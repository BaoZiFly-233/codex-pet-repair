...
impl super::WinitCompatibleRenderer for WinitSoftwareRenderer {
    fn render(&self, window: &i_slint_core::api::Window) -> Result<DrawOutcome, PlatformError> {
        ...
        // Windows can lose the backing pixels without invalidating buffer age.
        // Repaint the complete surface on a requested frame, including after
        // resize/occlusion. Do not add an idle redraw timer.
        ...
        if let Some((_w, _h)) = Option::zip(NonZeroU32::new(size.width), NonZeroU32::new(size.height))
        {
            winit_window.pre_present_notify();
            #[cfg(target_os = "windows")]
            target_buffer
                .present()
                .map_err(|e| format!("Error presenting complete softbuffer buffer: {e}"))?;
            #[cfg(not(target_os = "windows"))]
            {
    ...
    fn resume(
        ...
        // The decorated application window has an opaque background. Do not
        // opt it into Windows transparency; retain it for undecorated popups.
        #[cfg(target_os = "windows")]
        let window_attributes = if window_attributes.decorations {
            window_attributes.with_transparent(false)
        } else {
            window_attributes
        };
        let winit_window =
